...
However the POSIX spec is very lenient in these scenarios and many things that would be considered errors are ignored or resolved in some way. %XXX Give example
...
%Using Go allowed me to use an interface instead of a union to represent a node.
%This interface consists of a single method, Eval, which all the different nodes then satisfy allowing related code to be contained together.

As with most tree structures each node in the AST contains other nodes, the exception being the NodeCommand which is the terminal node that encompasses running a command, builtin or user defined function.
In the specification this node is referred to as a simple command.
Most of the other nodes are compound commands, grouping these simple commands in different ways.
The exception to this is the NodeRedirection which used to apply IO redirections to subsequent si 
