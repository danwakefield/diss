...
Shells can accept input in a variety of ways including reading from a pipe or a file and an interactive session.
I felt that the base case of these was file input as a pipe can be treated as a file.

After researching the complexities surrounding interactivity I decided it would have to be left out due to the time restrictions. The library\cite{GO-READLINE}



The first step we need to do is get the text that represents the shell program in some way.
This can come in from a 
...
There was also the choice of using a lexer generator such as ANTLR or Ragel but with Go being such a recent language these tools generally don't have good support for it, Ragel being an exception.
...
By itself or when followed by a invalid substitution character it becomes a literal character with no special meaning.

The lexer also handles other complex tasks such as command substitution lexing, which requires a sub lexer parser pair to evaluate correctly and alias expansion.
...
In conclusion the I went with a hand written lexer ...

learn the about their complexities without getting tool specific knowledge.

the fact that certain shell constructs would have been extremely difficult to encode using a tool

XXX %Lexer generators, ANTLR (Doesnt support GO), Ragel (Does), Hard to do things like substiutions embedded in strings etc. NDFA cite lexing video, Point to section implemtation/review about why this may have been a bad idea.
