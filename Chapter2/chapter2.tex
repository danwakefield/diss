...
The lexer returns items called Lexemes (See code sample~\ref{code:lexme} which contain the token type and any additional information.
...
Nodes in this tree satisfy an interface called Node, which contains a single method called Eval (See code in~\ref{code:node-interface}).
...
It is a subtype of the NodeNoop which is achieved by Go's struct embedding feature. (See code in~\ref{code:node-eof-noop})
