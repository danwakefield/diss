...
There was also the option of LALR or LR lookahead parsers and backtrakin
...
When evaluation hits a NodeCommand we know we are going to do either set some variables \lstinline!A=1 B=2! or run some sort of command, with optionally prepended assignments\lstinline!A=1 foo!.
In either case it is possible that a value exists that requires expansion e.g \lstinline!A="$X $(foo) $((3+4))" B=*.c!.
