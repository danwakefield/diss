...
Also known as Golang, it is a statically typed, compiled, concurrent, garbage collected language that is heavily inspired by C for the general syntax, Limbo for it's use of Communicating Sequential Processes (CSP)\cite{HOARE-CSP} and Python for its emphasis on readability and an all inclusive standard library.
It is a relatively modern language, released to the public in 2009, that was created to solve some of the problems experienced with the languages used at Google, namely C\verb!++!, Python and Java.
I began to learn the language, through the Golang Challenge\cite{GOLANG-CHALLENGE},  during my industrial year after experiencing some of these problems with python myself.
...
It was considered as a candidate as it is often said that creating an interpretor or compiler is easier in a ML inspired language than an imperative one.

I however had no experience at all with OCaml.
I considered it instead of other functional languages as it has recently been used to create some the high profile language Hack at Facebook.
As well as this prior art it has been used for the Haxe language, The MirageOS unikernel and many code analysis tools. comes with its own versions of the lex and yacc tools which are useful for creating a language using code generation.


It is also garbage collected, boasts an impressive standard library and has cross platform compilation abilities. 
The thing that reduces the interest in this language are the complete lack of experience using it and the fact it relies heavily on functional programming. 
Learning a new language and new programming paradigm does not seem like a sensible thing to do while working on such an important project.
