...
There are many tools available to generate parsers and Go even includes an implementation of yacc, one of the most famous, in its toolchain.
However I feared that using one would have taught me more about the tool than parsers in general.
This would have made debugging the generated code more difficult 

Bash uses bison to generate its parser but, Chet Ramey, its maintainer has stated that this was a bad decision and if he was starting over he would write a recursive descent parser himself\cite{BASH-ARCH}

I chose to write a predictive recursive descent parser for a few reasons.
First of all it works for the 
...
If the argument contains a slash it is taken to be a relative path and looked for from the current working directory.
\begin{enumerate*}
