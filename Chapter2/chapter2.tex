...
This means that the first word of any line should be checked against a list of expansions, if it matches the word is replaced by the expanded text and lexing begins at the start of the line again.

However as this modified example taken from bashes test suite shows this can result in a common compiler construction problem 
\begin{verbatim}
alias foo=bar
alias bar=baz
alias baz=foo
...
This is a common problem in compiler construction known as indirect left recursion and care must be taken to avoid infinite loops.
