...
Simple ones in the fourth iteration alongside extensions to the parse.
...
`foo \` bar \\\`baz\\\` \` `  # Old
\end{verbatim}
The code above shows how you nest three levels using each type of subshell.
To parse a subshell that uses graves you must find the correct end grave and then remove a level of escaping for 
every subshell it contains before passing it into a new lexer.
