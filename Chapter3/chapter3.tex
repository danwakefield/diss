...
\section{Expansion}
Expansions are performed in a few places throughout the code though most require only a subset of the possible ones.
Pattern matching for the case statement should only perform expansion of variables.
During expansion. 

as the last step before a command executes.
They could also occur when assigning to a variable with an integer type.
They are triggered by a call to Expand.
This is done as expansions are reevaluated on each execution. 
There are some places in the code where expansions are desired 
