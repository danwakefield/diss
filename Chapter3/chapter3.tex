...
The fix for this would be very simple, it just requires the addition of a division helper function similar to that used for shifting (See \ref{lst:arith-shift}). XXX % Add reference to integration test that caught it.
...
Variable names are detected using the following simple regular expression, '[a-zA-Z_][a-zA-Z0-9_]*'.
...
Symbols are the simplest thing to detect, See one and return the Token representing them.

\subsection{Parser}

\subsection{Variables}
--- Follow the simple regex . The arith construct cannot access the values of special variables such as '\$#'
