...
Every Linux distribution right down to the very smallest\cite{ALPINE-LINUX} include a shell of some form that complies with the POSIX shell specification\cite{POSIX-SHELL-STANDARD}. The Almquist Shell(ash), an implementation of which is included in the busybox binary, and the Debian Almquist Shell(dash) are minimal implementations of the spec.

Other common shells such as bash provide options 
