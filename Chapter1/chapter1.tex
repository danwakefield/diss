...
There are two families of shells, those descended from sh and csh, (See figure~\ref{fig:shell-history}), known as traditional shells and those that have a largely incompatible syntax and/or behaviour known as exotic shells. Fish\cite{FISH}, es\cite{ES-SHELL} and oh\cite{OH-SHELL} are examples of exotic shells.

Every Linux distribution right down to the very smallest\cite{ALPINE-LINUX} include a shell of some form that complies with the POSIX shell specification\cite{POSIX-SHELL-STANDARD}.
